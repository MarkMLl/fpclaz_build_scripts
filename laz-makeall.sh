...
#    PACKAGENAMES+=,anchordockingdsgn
  ...
  if [ -n "$USE_OPENGL" ]; then
    PACKAGENAMES+=,lazopenglcontext
  fi # USE_OPENGL

# Note LazMapViewer revision number below.

  if [ -n "$USE_LAZMAPVIEWER" ]; then
#    LMV_REV=r9394
    LMV_REV=r9774
    PACKAGENAMES+=,$PACKAGEPATH/$LMV_REV-components-lazmapviewer/lazmapviewer_bgra.lpk
    PACKAGENAMES+=,$PACKAGEPATH/$LMV_REV-components-lazmapviewer/lazmapviewerpkg.lpk
    PACKAGENAMES+=,$PACKAGEPATH/$LMV_REV-components-lazmapviewer/lazmapviewer_rgbgraphics.lpk
    PACKAGENAMES+=,$PACKAGEPATH/$LMV_REV-components-lazmapviewer/lazmapviewer_synapse.lpk
...
# consistency, I continue to use comma-separation.
    ...
    # In the specific case of a project using Synapse and OpenSSL, these two files
    # need to be copied or symlinked:

    SYN=$PACKAGEPATH/laz_synapse
    LMV=$PACKAGEPATH/$LMV_REV-components-lazmapviewer
